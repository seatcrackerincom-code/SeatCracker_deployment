  ...
  useEffect(() => {
    ...
    Promise.all(
      subjects.map(async (sub) => {
        try {
          const res = await fetch(`/EAMCET/SYLLABUS/${exam}/${course}/${sub}/${sub}.json`);
  ...
  const startExamWith = useCallback(async (subject: string, topic: string) => {
    ...
    try {
      ...
      const fetchPath = isRestructured 
        ? `/EAMCET/questions_v2/${folderName}/${topicSlug}/attempt_${selectedAttempt}.json`
        : `/EAMCET/questions_v2/${folderName}/${topicSlug}.json`;
