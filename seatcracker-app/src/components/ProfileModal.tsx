              ...
              {editingName ? (
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
                  <h2 style={{ fontSize: "22px", fontWeight: "800", color: "#fff", margin: 0 }}>
                    {authUser?.displayName || (authUser?.email ? (authUser.email.split('@')[0].length >= 2 ? authUser.email.split('@')[0].substring(0, 2).toUpperCase() : authUser.email.split('@')[0].toUpperCase()) : (user.name !== "Learner" ? user.name : "SC"))}
                  </h2>
